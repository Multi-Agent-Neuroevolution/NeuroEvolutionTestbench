...
class MovementState(TaskState):
    def __init__(self, agents: List[Agent], bounds: Tuple[float, float, float, float]):
        self.agents = agents
        self.bounds = bounds # (min_x, max_x, min_y, max_y); these are the boundaries, self-explanatory
        self.velocities = {agent.id: np.zeros(2) for agent in agents} # Saves individual vel. values per agent
...
class MovementHandler:
    ...
    def get_distances(self, state: MovementState) -> np.ndarray:
        ...
        distances = np.linalg.norm(positions[:, np.newaxis] - positions, axis=2) # Here distances[i, j] means distance between agent i and agent j
        return distances
...
def breed_and_mutate(config, parents, num_offspring, bounds, multi_model):
    ...
    for _ in range(num_offspring):
        ...
        if multi_model == True:
            ...
            if len(parents) == 0:
                child_genome.configure_new(configCopy.genome_config)
            else:
                parent1 = random.choice(parents)
                parent2 = random.choice(parents)
                child_genome.configure_crossover(
                    parent1.neat_genome, parent2.neat_genome, configCopy)
                child_genome.mutate(configCopy.genome_config)

            # This determines whether the new agent is a predator or prey
            pos = [random.uniform(bounds[0], bounds[1]),
                   random.uniform(bounds[0], bounds[1])]

            if isinstance(parent1, Predator):

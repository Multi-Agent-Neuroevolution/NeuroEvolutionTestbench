...
class Obstacle(Shape):
    def __init__(self, pos, shape, radius, width, height, color, interactible, collidable, goal_obstacle):
        super().__init__(shape=shape, radius=radius, width=width,
                         height=height, pos=pos, collidable=collidable)
        self.interactible = interactible
        # goal_obstacle is used to determine if the obstacle is a goal for the agent. NOT USED FOR PREDATOR-PREY
        self.goal_obstacle = goal_obstacle
        # TO DO: Move color to Shape() class, then adjust all subclasses to account for that
        self.color = color
        self.living = True
...
class Food(Obstacle):
    def __init__(self, pos):
        super().__init__(pos=pos, shape="Circle", radius=1, width=0, height=0,
                         color="green", interactible=True, collidable=False, goal_obstacle=False)
